...
export default function HistoryPage() {
  ...
  if (outcomeFilter !== 'all')
    list = list.filter((b) => b.outcome === outcomeFilter);
  if (sportFilter !== 'all')
    list = list.filter((b) => b.sport_icon === sportFilter);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-bg-2 border border-border rounded p-3 flex gap-2 flex-wrap items-center">
        ...
        <span className="ml-auto text-txt-3 text-xs">
          {list.length} apostas
        </span>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left text-txt-3 text-[10px] uppercase tracking-wider px-4 py-3 font-normal"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="text-center py-10 text-txt-3">
                    Carregando...
                  </td>
                </tr>
              ) : !list.length ? (
                ...
                list.map((b) => {
                  const icons = Array.from(
                    new Set(b.items.map((i: any) => i.sport_icon)),
                  ).join('');
                  const realReturn =
                    b.outcome === 'won'
                      ? (b.real_return ?? b.potential_return)
                      : b.outcome === 'lost'
                        ? 0
                        : null;
                  const matchLabel =
                    b.items.length > 1
                      ? `${b.items.length} seleções`
                      : (b.items[0] as any)?.match;
                  const typeLabel =
                    b.items.length > 1 ? 'Múltipla' : (b.items[0] as any)?.type;

                  return (
                    <tr
                      key={b.id}
                      className="border-b border-border last:border-0 hover:bg-white/[0.015] transition-colors"
                    >
                      ...
                      <td className="px-4 py-3 text-txt-2 max-w-[140px] truncate">
                        {typeLabel}
                      </td>
                      <td className="px-4 py-3 font-medium text-white">
                        {b.total_odd.toFixed(2)}
                      </td>
                      <td className="px-4 py-3 text-txt-2">
                        {formatCurrency(b.stake)}
                      </td>
                      <td
                        className={`px-4 py-3 font-medium ${b.outcome === 'won' ? 'text-success' : b.outcome === 'lost' ? 'text-danger' : 'text-txt-3'}`}
                      >
                        ...
                        <div className="flex gap-1">
                          {b.outcome === 'pending' ? (
                            <>
                              <Button
                                onClick={() =>
                                  updateOutcome(b.id, 'won', b.potential_return)
                                }
                              ...
                              <Button
                                variant="danger"
                                onClick={() => updateOutcome(b.id, 'lost')}
                                className="py-0.5 px-1.5 text-[10px]"
                              >
                                <X size={10} />
                                Perdeu
