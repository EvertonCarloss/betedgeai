...
export function BetSlip() {
  ...
  const icons = [...new Set(slip.map(b => b.sport_icon))]
  ...
  async function confirm() {
    ...
    try {
      await addBet({
        items: slip.map(b => ({ match: b.match, type: b.type, odd: b.odd, sport_icon: b.sport_icon })),
      ...
      {!slip.length ? (
        ...
        <>
          <div className="space-y-2 mb-3">
            {slip.map(b => (
              <div key={b.id} className="bg-bg border border-border rounded p-2.5 relative pr-7">
                <div className="font-display font-bold text-white text-xs">{b.match}</div>
                <div className="text-txt-2 text-[11px] mt-0.5">{b.type}</div>
                <div className="text-accent text-xs font-medium mt-1">{b.odd.toFixed(2)}</div>
                <button onClick={() => removeFromSlip(b.id)} className="absolute top-2 right-2 text-txt-3 hover:text-danger transition-colors">
                  <X size={12} />
                </button>
              </div>
            ))}
          ...
          <div className="border-t border-border pt-3 space-y-2">
            <div className="flex justify-between text-xs text-txt-2">
              <span>Odd total</span>
              <span className="text-white font-medium">{totalOdd.toFixed(2)}</span>
            </div>
            <div>
              <label className="text-[10px] text-txt-3 uppercase tracking-wider">Stake (R$)</label>
              <input
            ...
            <div className="flex justify-between text-xs text-txt-2">
              ...
              <span className="text-success font-display font-bold text-base">{formatCurrency(potReturn)}</span>
            </div>

            {success && (
              <div className="bg-success/10 border border-success/30 rounded p-2 text-success text-xs text-center">
                <Check size={12} className="inline mr-1" />Aposta registrada!
              </div>
            )}

            <Button variant="primary" className="w-full justify-center" onClick={confirm} disabled={loading}>
              <Check size={13} />
              {loading ? 'Salvando...' : 'Confirmar aposta'}
            </Button>
            <Button variant="danger" className="w-full justify-center" onClick={clearSlip}>
